var search_input = document.querySelector("#search_input")
        ...
        search_input.addEventListener("keyup", function(e){
            ...
            for(var i=0; i<span_items_dateService.length; i++){
                if((span_items_dateService[i].textContent.toLowerCase().indexOf(search_item)&&span_items_nameOfService[i].textContent.toLowerCase().indexOf(search_item)&&span_items_id[i].textContent.toLowerCase().indexOf(search_item)&&span_items_createdTask[i].textContent.toLowerCase().indexOf(search_item)&&span_items_adressOfService[i].textContent.toLowerCase().indexOf(search_item)) != -1){ //indexOf czyli zwraca gdzie w wyszukiwanym elemencie jest wyszukiwana fraza, -1 nie ma te frazy w wyszukiwanym
...
//skracanie nazwy <a></a> i dodawanie obrazów do plików
...
function letters_last_name_get_color(){
    
    var letters_class = document.querySelectorAll('.tasks-details .task .letters')
    var clients_Last_Name = document.querySelector('.details-clients-tasks .data-client-outside .data-client1 .clientLastName span')
    ...
    var first_letter = clients_Last_Name.innerHTML.slice(0,1).toLowerCase()
